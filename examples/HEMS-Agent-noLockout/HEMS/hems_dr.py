...
def hems_dr(config_path, **kwargs):
    ...
    class HemsDR(Agent):
        ...
        def subscribeHouseValues(self, peer, sender, bus,  topic, headers, message):
            if (topic == 'analysis/controls/ecobeeCurrent/currentTemp'):
                self.T_a = message[0]["currentTemp"]
		print(self.T_a)
            if (topic == 'devices/LabHomes/controlsHome/campbell-TC-B/all'):
                self.Q_s = message[0]['Solar Outside Modbus']
		self.T_out = message[0]['Outside Air Temperature RH']
		print(self.Q_s)
		print(self.T_out)
            #if (topic == 'devices/LabHomes/controlsHome/campbell-TC-B/Outside Air Temperature RH'): 
            #    self.T_out = message[0]
            if (topic == 'devices/LabHomes/controlsHome/campbell-PW-B/all'): 
                self.HP_power = message[0]['Heat Pump Total Power']
		print(self.HP_power)
		...
		print(self.DR)
            ...
            self.Q_limvalue = 1700 * (percentage/100)
        ...
        def DRStartController(self):
            if (self.DR == True):
                ...
                if (self.HP_power < 50):
                    ...
                    Dstatus_AC_dn = 1
		#print(P_avg, P_sigma, P_cap, para_AC_dn, self.T_a,halfband_AC_dn_above,halfband_AC_dn_below,1, mdt, A, B, C,D, self.T_out, Q_i, self.Q_s,-8616.29,ddt)
		...
		flexibility_message = [{"P_min" : P_min, "Q_min":Q_min,"P_max":P_max,"Q_max":Q_max },{"P_min":{'units': 'F','tz':'UTC','type':'float'}, "Q_min":{'units': 'F','tz':'UTC','type':'float'},"P_max":{'units': 'F','tz':'UTC','type':'float'}, "Q_max":{'units': 'F','tz':'UTC','type':'float'}}]
            	flexibility_pub_topic = 'analysis/controls/ACflexibility/value'
                self.vip.pubsub.publish('pubsub',flexibility_pub_topic,headers,flexibility_message).get(timeout=2)

		market_message = [{"P_clear" : P_clear, "Q_clear":Q_clear, "Q_lim": self.Q_limvalue },{"P_clear":{'units': 'F','tz':'UTC','type':'float'}, "Q_clear":{'units': 'F','tz':'UTC','type':'float'},"Q_lim":{'units': 'F','tz':'UTC','type':'float'}}]
            	market_pub_topic = 'analysis/controls/market/clear'
            	self.vip.pubsub.publish('pubsub',market_pub_topic,headers,market_message).get(timeout=2)

                #P_clear = market_clear[0]
                #Q_clear = market_clear[1]
		print(self.Q_limvalue)
		print(P_clear)
		print(Q_clear)
        ...
        def publishValue(self,value):
            ...
            T_set_AC_dn_message = [{"setpoint" : value},{"setpoint":{'units': 'F','tz':'UTC','type':'float'}}]
